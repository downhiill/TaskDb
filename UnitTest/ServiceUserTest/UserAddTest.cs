    ...
    public class UserAddTest : ServiceUsersTests
    {
        ...
        public void Add_ShouldAddUser(UserModel user, bool expectedSuccess)
        {
            ...
            if (!expectedSuccess)
            {
                ...
                {
                    // Если имя пустое или состоит из пробела/табуляции, проверяем, что выбрасывается ArgumentException
                    Assert.IsType<ArgumentException>(exception);
                    Assert.Equal("User name cannot be empty or whitespace.", exception.Message);
                }
                else
                {
                    // Если имя корректное, проверяем, что выбрасывается InvalidOperationException
